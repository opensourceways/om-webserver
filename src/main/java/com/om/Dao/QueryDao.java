...
//                }
    ...
    public String querySigName(String community, String lang) {
        try {
            AsyncHttpClient client = AsyncHttpUtil.getClient();
            RequestBuilder builder = asyncHttpUtil.getBuilder();
            String index = "";
            ...
            switch (community.toLowerCase()) {
                ...
                case "opengauss":
                    ...
                    break;
                case "mindspore":
                    return getMindsporeSigList(lang);
                default:
                    return "{\"code\":" + 404 + ",\"data\":{\"sigs\":" + 0 + "},\"msg\":\"not Found!\"}";
            }
            builder.setUrl(this.url + index + "/_search");
...
//                        company.contains("软通动力") ||
    ...
    public String queryMetricIncreaseContribute(String community, long start, long end, DatastatRequestBody body, String interval, String userQuery) {
        ...
        try {
            ...
            String term = (String) variables.get("term"); // term: D1 huawei sig repo
            ...
            } else if (term.equalsIgnoreCase("D1") || term.equalsIgnoreCase("D2")) {
    ...
    public ArrayList<HashMap<String, Object>> getResponseResult(AsyncHttpClient client, RequestBuilder builder, String index, String query) {
    ...
    public String queryMetricTotalCount(String community, long start, long end, DatastatRequestBody body, String userQuery) {
        ...
        if (variables.containsKey("sig")) {
            return queryMetricTotalCountSigOwner(community, start, end, body);
        }
        if (variables.containsKey("term")) {
            String term = (String) variables.get("term");
            if (term.equalsIgnoreCase("sig") || term.equalsIgnoreCase("repo") || companyQueryMap.has(term.toLowerCase())) {
                return queryMetricTotalCountTermDetails(community, start, end, body, userQuery, term);
            }
        ...
        try {
            ...
            for (String metric : metrics) {
                if (metric.equals("download_ip")) {
                    int cnt = queryMetricDownloadIPCount(community, start, end, body);
                    data.put(metric, cnt);
                } else if (metric.equals("download_count")) {
                    int cnt = queryMetricDownloadCount(community, start, end, body);
                    data.put(metric, cnt);
                } else {
                    String userquery = userQueryMap.get(metric).asText();
                    String query = String.format(queryjson, start, end, convertList2queryStr(internals),
                    ...
                    String responseBody = f.get().getResponseBody(UTF_8);
                    JsonNode dataNode = objectMapper.readTree(responseBody);
                    Iterator<JsonNode> buckets = dataNode.get("aggregations").get("group_field").get("buckets")
                            .elements();
                    ArrayList<HashMap<String, Object>> list = new ArrayList<>();
                    while (buckets.hasNext()) {
                        JsonNode bucket = buckets.next();
    ...
    public String queryMetricUserActiveCountDetail(String community, long start, long end, DatastatRequestBody body, String userQuery) {
        String index;
        String queryjson;
        switch (community.toLowerCase()) {
            case "openeuler":
                index = openEuler.getGiteeAllIndex();
                queryjson = openEuler.getuserActiveDetailquery();
                break;
            default:
                return "{\"code\":400,\"data\":\"query error\",\"msg\":\"query error\"}";
        }
    ...
    public String queryMetricUserCountDetail(String community, long start, long end, DatastatRequestBody body) {
        ...
        switch (community.toLowerCase()) {
            case "openeuler":
                index = openEuler.getuserCountIndex();
                queryjson = openEuler.getuserCountDetailquery();
                break;
            default:
                return "{\"code\":400,\"data\":\"query error\",\"msg\":\"query error\"}";
        }
        return queryMetricUserDetailCommon(index, queryjson, start, end, body, null);
    }

    public String queryMetricUserDetailCommon(String index, String queryjson, long start, long end, DatastatRequestBody body, String userQuery) {
        try {
            HashMap<String, Object> variables = body.getvariables();
            ArrayList<String> orgs = (ArrayList<String>) variables.get("org"); // openeuler,src-openeuler
            ArrayList<String> internals = (ArrayList<String>) variables.get("internal"); // 0,1
            ...
            for (String metric : metrics) {
                ...
                String query = String.format(queryjson, start, end, convertList2queryStr(internals), convertList2queryStr(orgs), userquery);
                builder.setUrl(this.url + index + "/_search");
                builder.setBody(query);
                ListenableFuture<Response> f = client.executeRequest(builder.build());
                String responseBody = f.get().getResponseBody(UTF_8);
    ...
    public int queryMetricDownloadIPCount(String community, long start, long end, DatastatRequestBody body) {
        String index;
        String queryjson;
        int cnt = 0;
        switch (community.toLowerCase()) {
            case "openeuler":
                ...
                queryjson = openEuler.getdownload_ip_count_query();
                break;
            default:
                return cnt;
        }
        try {
            ...
            String query = String.format(queryjson, start, end);
            builder.setUrl(this.url + index + "/_search");
            builder.setBody(query);
            ListenableFuture<Response> f = client.executeRequest(builder.build());
            String responseBody = f.get().getResponseBody(UTF_8);
            ...
            while (buckets.hasNext()) {
                JsonNode bucket = buckets.next();
                cnt = bucket.get("res").get("value").asInt();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public int queryMetricDownloadCount(String community, long start, long end, DatastatRequestBody body) {
        int ans = 0;
        try {
            AsyncHttpClient client = AsyncHttpUtil.getClient();
            RequestBuilder builder = asyncHttpUtil.getBuilder();
            ...
            switch (community.toLowerCase()) {
                case "openeuler":
                    index = openEuler.getUsers_index();
                    queryjson = openEuler.getdownload_queryStr();
                    String[] queryjsons = queryjson.split(";");
                    HashMap<String, Object> variables = body.getvariables();
                    ArrayList<String> oversea = (ArrayList<String>) variables.get("oversea");
                    for (int i = 0; i < queryjsons.length; i++) {
                        queryjson = queryjsons[i];
                        queryjson = String.format(queryjsons[i], start, end, convertList2queryStr(oversea), "10000d");
        ...
        try {
            ...
            switch (community.toLowerCase()) {
                case "openeuler":
                    ...
                    HashMap<Long, Double> dataincreasemap = new HashMap<>();
                    HashMap<Long, Double> datatotalmap = new HashMap<>();
                    for (int i = 0; i < queryjsons.length; i++) {
                        queryjson = queryjsons[i];
                        queryjson = String.format(queryjsons[i], start, end, convertList2queryStr(oversea), interval);
                        ...
                        while (buckets.hasNext()) {
                            JsonNode bucket = buckets.next();
                            Long date = bucket.get("key").asLong();
                            Double cur = bucket.get("res").get("value").asDouble();
                            total += cur;
                            if (dataincreasemap.containsKey(date)) {
                                dataincreasemap.put(date, dataincreasemap.get(date) + cur);
                            } else {
                                dataincreasemap.put(date, cur);
                            }
                            if (datatotalmap.containsKey(date)) {
                                datatotalmap.put(date, datatotalmap.get(date) + total);
                            } else {
                                datatotalmap.put(date, total);
                            }
                        }
                    }
                    dataincreasemap.forEach((key, value) -> {
                        HashMap<String, Object> tmp = new HashMap<>();
                        tmp.put("date", key);
                        tmp.put("increase", value);
                        tmp.put("total", datatotalmap.get(key));
                        res.add(tmp);
                    });
                    Collections.sort(res, new Comparator<HashMap<String, Object>>() {
                        @Override
                        public int compare(HashMap<String, Object> t1, HashMap<String, Object> t2) {
                            return t1.get("date").toString().compareTo(t2.get("date").toString());
                        }
                    });
                    return res;
            }
    ...
    public ArrayList<String> getTermQuery(String term) {
    ...
    public String queryMetricIncreaseTermContribute(String community, long start, long end, DatastatRequestBody body,
        ...
        switch (community.toLowerCase()) {
            case "openeuler":
                index = openEuler.getGiteeAllIndex();
                queryjson = openEuler.getAggContributeDetailquery();
                break;
            default:
                return "{\"code\":400,\"data\":\"query error\",\"msg\":\"query error\"}";
        }
        try {
            ...
            for (String metric : metrics) { // pr issue comment
                ...
                String userquery = userQueryMap.get(metric).asText();
                String termQuery = getTermQuery(term).get(0);
                String query = String.format(queryjson, start, end, convertList2queryStr(internals), convertList2queryStr(orgs), termQuery, interval, userquery);
                builder.setUrl(this.url + index + "/_search");
                builder.setBody(query);
                ListenableFuture<Response> f = client.executeRequest(builder.build());
                String responseBody = f.get().getResponseBody(UTF_8);
                ...
                while (buckets.hasNext()) {
                    JsonNode bucket = buckets.next();
                    String sig = bucket.get("key").asText();
                    Iterator<JsonNode> sigdetails = bucket.get("2").get("buckets").elements();
                    ArrayList<HashMap<String, Object>> tmp = new ArrayList<>();
                    long total = 0;
                    while (sigdetails.hasNext()) {
                        JsonNode sigdetail = sigdetails.next();
                        long date = sigdetail.get("key").asLong();
                        long increase = sigdetail.get("res").get("value").asLong();
                        total += increase;
                        HashMap<String, Object> item = new HashMap<>();
                        item.put("date", date);
    ...
    public String queryMetricTotalCountTermDetails(String community, long start, long end, DatastatRequestBody body,
            String userQuery, String term) {
        String index;
        String queryjson;
        try {
            ...
            for (String metric : metrics) {
                if (metric.equalsIgnoreCase("users")) {
                    ArrayList<HashMap<String, Object>> userDetails = queryMetricTotalCountUser(community, start, end, body, userQuery, term);
                    res.put(metric, userDetails);
                }
                if (metric.equalsIgnoreCase("contributes")) {
                    ArrayList<HashMap<String, Object>> contributeDetails = queryMetricTotalCountTermContribute(community, start, end, body, userQuery, term);
                    res.put(metric, contributeDetails);
                }
    ...
    public ArrayList<HashMap<String, Object>> queryMetricTotalCountTermContribute(String community, long start, long end, DatastatRequestBody body,
            String userQuery, String term) {
        String index;
        String queryjson;
        ArrayList<HashMap<String, Object>> res = new ArrayList<>();
        switch (community.toLowerCase()) {
            case "openeuler":
                index = openEuler.getGiteeAllIndex();
                queryjson = openEuler.getAggTotalContributeDetailquery();
                break;
            default:
                return res;
        }
        try {
            ...
            RequestBuilder builder = asyncHttpUtil.getBuilder();
            String termQuery = getTermQuery(term).get(0);
            String enterpriseQuery = getTermQuery(term).get(1);
            String query = String.format(queryjson, start, end, convertList2queryStr(internals), convertList2queryStr(orgs), enterpriseQuery, termQuery);
            builder.setUrl(this.url + index + "/_search");
            builder.setBody(query);
            ListenableFuture<Response> f = client.executeRequest(builder.build());
            String responseBody = f.get().getResponseBody(UTF_8);
    ...
    public ArrayList<HashMap<String, Object>> queryMetricTotalCountUser(String community, long start, long end, DatastatRequestBody body,
            String userQuery, String term) {
        ArrayList<HashMap<String, Object>> res = new ArrayList<>();
        String termQuery = getTermQuery(term).get(0);
        String enterpriseQuery = getTermQuery(term).get(1);
        ...
        for (String metric: metrics) {
            HashMap<String, Integer> metricData = queryMetricTotalCountTermUser(community, start, end, body, userQuery, metric, enterpriseQuery, termQuery);
    ...
    public HashMap<String, Integer> queryMetricTotalCountTermUser(String community, long start, long end, DatastatRequestBody body,
            String userQuery, String metric, String enterpriseQuery, String termQuery) {
        ...
        switch (community.toLowerCase()) {
            case "openeuler":
                index = openEuler.getGiteeAllIndex();
                queryjson = openEuler.getAggTotalUserCountquery();
                break;
            default:
                return res;
        }
        try {
            ...
            String userquery = userQueryMap.get(metric).asText();
            String query = String.format(queryjson, start, end, convertList2queryStr(internals),
                    convertList2queryStr(orgs), userquery, enterpriseQuery, termQuery);
            builder.setUrl(this.url + index + "/_search");
            builder.setBody(query);
            ListenableFuture<Response> f = client.executeRequest(builder.build());
            String responseBody = f.get().getResponseBody(UTF_8);
    ...
    public String queryMetricTotalCountSigOwner(String community, long start, long end, DatastatRequestBody body) {
        HashMap<String, Object> res = new HashMap<>();
        String index;
        String queryjson;
        switch (community.toLowerCase()) {
        ...
        try {
            ...
            AsyncHttpClient client = AsyncHttpUtil.getClient();
            RequestBuilder builder = asyncHttpUtil.getBuilder();
            String query = String.format(queryjson, start, end, convertList2queryStr(internals), sig);
            builder.setUrl(this.url + index + "/_search");
            builder.setBody(query);
            ListenableFuture<Response> f = client.executeRequest(builder.build());
            String responseBody = f.get().getResponseBody(UTF_8);
