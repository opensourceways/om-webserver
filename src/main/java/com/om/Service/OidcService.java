...
import com.om.authing.SessionManager;
...
public class OidcService {
    ...
    public ResponseEntity oidcAuthorize(HttpServletRequest servletRequest, HttpServletResponse servletResponse) {
        try {
            ...
            // 已经登录，则不弹出登录界面，直接跳转获取token
            String checkResult = sessionManager.checkSession(servletRequest, servletResponse, false, false);
            if (Constant.SUCCESS.equals(checkResult) && redirect2Token(servletRequest, servletResponse)) {
    ...
    /**
     * 重定向到三方服务的rediredcturi
     *
     * @param servletRequest  请求体
     * @param servletResponse 响应体
     * @return 是否重定向
     */
    private boolean redirect2Token(HttpServletRequest servletRequest, HttpServletResponse servletResponse) {
        boolean ret = false;
        try {
            Cookie tokenCookie = sessionManager.verifyCookie(servletRequest);
            if (tokenCookie == null) {
                return ret;
            }
            ...
            String scope = parameterMap.getOrDefault("scope", new String[]{""})[0];
            String state = parameterMap.getOrDefault("state", new String[]{""})[0];
            ResponseEntity response = oidcAuth(token, clientId, redirectUri, responseType, state, scope);
            if (HttpStatus.OK.equals(response.getStatusCode())) {
                Object responseBody = response.getBody();
                String redirectTokenUrl = "";
                if (responseBody instanceof HashMap) {
                    HashMap<String, String> responseObj = (HashMap<String, String>) responseBody;
                    redirectTokenUrl = responseObj.get("body");
                } else {
                    LOGGER.error("parse code url failed");
                    return ret;
                }
                servletResponse.sendRedirect(redirectTokenUrl);
                ret = true;
            }
        } catch (IOException e) {
            LOGGER.error("redirect to token_url failed {}", e.getMessage());
        } catch (Exception e) {
            LOGGER.error("redirect to token_url failed {}", e.getMessage());
        }
        return ret;
